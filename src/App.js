...
var AT={
  ...
  uploadFile:async function(recordId,file){
    ...
    var r=await fetch("https://content.airtable.com/v0/"+AT_BASE+"/"+recordId+"/uploadAttachment",{
...
function Dashboard(){
  ...
  jobs.forEach(function(j){
    var s=j.Status||"Unknown";bS[s]=(bS[s]||0)+1;
    bD[j.Department||"?"]=(bD[j.Department||"?"]||0)+1;
    bT[j.JobType||"?"]=(bT[j.JobType||"?"]||0)+1;
    var pi=j.FacultyName||"?";if(!bP[pi])bP[pi]={j:0,h:0};bP[pi].j++;
    var h=parseFloat(j.HoursEst)||0,pr=parseFloat(j.PartsEst)||0;
    tH+=h;tP+=pr;bP[pi].h+=h;
    if(s==="Approved"){aH+=h;aP+=pr;aC++;}
    if(s==="Complete"){
      tAH+=parseFloat(j.ActualHours)||0;
      tAP+=parseFloat(j.ActualParts)||0;
      cC++;
    }
  });
  var SC={"Pending Shop Review":"#c8a951","Pending PI Approval":"#2563eb","Approved":"#1a5c2a","Needs Clarification":"#7a4f00","Complete":"#5c1212"};
  ...
  var actR=Math.round(tAH*75)+Math.round(tAP);
  ...
  return<div>
    <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:16,marginBottom:28}}>
      ...
      <St l="Total Est. Revenue" v={fmt$(totR)} s={cC>0?"Actual: "+fmt$(actR)+" ("+cC+" jobs)":aC+" approved ("+fmt$(appR)+")"} c="var(--green)"/>
    </div>
    {cC>0&&<div style={{background:"var(--ml)",border:"1px solid #e8d4d4",borderRadius:"var(--r)",padding:"16px 20px",marginBottom:24,fontFamily:"'DM Mono',monospace",fontSize:13}}>
      <span style={{color:"var(--m)",fontWeight:600}}>Variance ({cC} completed jobs): </span>
      <span>Labor: est {(tAH).toFixed(1)}h actual vs {(tH).toFixed(1)}h est — </span>
      <span>Revenue: actual {fmt$(actR)} vs est {fmt$(totR)} — </span>
      <span className={(actR<totR*0.9)?"var-neg":"var-pos"}>{actR<totR*0.9?"Under-reported hours detected":"On track"}</span>
    </div>}
    ...
    <div className="card"><div className="ct">All Jobs</div><div style={{overflowX:"auto"}}>
      <table style={{width:"100%",borderCollapse:"collapse",fontSize:13}}>
        ...
        <tbody>{[...jobs].sort(function(a,b){return(b.Timestamp||"").localeCompare(a.Timestamp||"");}).map(function(j,i){
          ...
          var estTot=(parseFloat(j.HoursEst)||0)*75+(parseFloat(j.PartsEst)||0);
          var actTot=(parseFloat(j.ActualHours)||0)*75+(parseFloat(j.ActualParts)||0);
          var variance=j.ActualHours?Math.round(actTot-estTot):null;
          return<tr key={i} style={{borderBottom:"1px solid var(--bo)"}}>
            ...
            <td style={{padding:"8px 10px",fontFamily:"'DM Mono',monospace",fontSize:11,whiteSpace:"nowrap"}}>{variance!==null?<span className={variance>0?"var-neg":"var-pos"}>{variance>0?"+":""}{fmt$(variance)}</span>:"—"}</td>
...
function IntakeForm(){
  ...
  async function sub(e){e.preventDefault();sSt("s");try{
    ...
    if(file&&rec.id){await AT.uploadFile(rec.id,file);}
  ...
  return<form onSubmit={sub}>
    ...
    <div className="card"><div className="ct">📎 Drawings / Attachment (optional)</div>
      <div className={"upload-area"+(drag?" drag":"")}
        onDragOver={function(e){e.preventDefault();sDrag(true);}}
        onDragLeave={function(){sDrag(false);}}
        onDrop={function(e){e.preventDefault();sDrag(false);var fl=e.dataTransfer.files&&e.dataTransfer.files[0];if(fl)sFile(fl);}}
...
function PIApproval(P){
  ...
  async function sub(e){e.preventDefault();if(!dec)return;sSt("s");try{
    ...
    var j=rec.fields;
    var hrs=parseFloat(j.HoursEst)||0,pts=parseFloat(j.PartsEst)||0;
    var compLk=ou()+"/#complete?job="+jobId;
    var shopMsg="PI "+j.FacultyName+": "+sv2+(ok&&spd?"\nSpeedtype: "+spd:"")+(com?"\nComments: "+com:"")+"\nJob: "+j.JobName+(ok?"\n\nMark job complete when done:\n"+compLk:"");
    await mail(SHOP,"[Shop] "+jobId+" — "+sv2,shopMsg);
...
function JobComplete(P){
  ...
  async function sub(e){e.preventDefault();sSt("s");try{
    var j=rec.fields;
    var estTot=(parseFloat(j.HoursEst)||0)*75+(parseFloat(j.PartsEst)||0);
    var actTot=(parseFloat(ah)||0)*75+(parseFloat(ap)||0);
    var variance=Math.round(actTot-estTot);
    await AT.update(rec.id,{Status:"Complete",ActualHours:ah,ActualParts:ap,CompletedDate:cd,CompletionNotes:cn});
    var msg="Job "+jobId+" marked complete.\n\nJob: "+j.JobName+"\nSubmitter: "+j.SubmitterName+"\nPI: "+j.FacultyName+"\n\nEstimate vs Actual:\nLabor est: "+j.HoursEst+"h — Actual: "+ah+"h\nParts est: "+fmt$(j.PartsEst||0)+" — Actual: "+fmt$(ap||0)+"\nRevenue est: "+fmt$(estTot)+" — Actual: "+fmt$(actTot)+"\nVariance: "+(variance>=0?"+":"")+fmt$(variance)+(cn?"\n\nNotes: "+cn:"");
  ...
  if(!rec)return<div className="err" style={{marginTop:40}}>Job {jobId} not found.</div>;
  var j=rec.fields;
  if(j.Status==="Complete")return<div className="done"><div className="done-i">🏁</div><h2 className="done-t">Already Marked Complete</h2><div className="bid">{jobId}</div><div style={{marginTop:16,fontFamily:"'DM Mono',monospace",fontSize:13,color:"var(--mu)"}}><div>Actual hours: {j.ActualHours}h</div><div>Actual parts: {fmt$(j.ActualParts||0)}</div><div>Completed: {j.CompletedDate}</div></div></div>;
  ...
  var variance=Math.round((actH*75+actP)-(estH*75+estP));
  return<form onSubmit={sub}>
    <div className="js"><div className="jst">Closing out — {jobId}</div><div className="jsg">
      ...
      <div className="ji"><span>Est. total</span>{fmt$(estH*75+estP)}</div><div className="ji"><span>Speedtype</span>{j.Speedtype||"—"}</div>
    </div></div>
    <div className="card"><div className="ct">✔️ Actual Costs</div>
      <div className="fr">
        <div className="fi"><label>Actual hours <span className="rq">*</span></label><input required type="number" min="0" step="0.5" value={ah} onChange={function(e){sAh(e.target.value);}}/><p className="hp">Total hours worked</p></div>
        <div className="fi"><label>Actual parts cost ($)</label><input type="number" min="0" step="0.01" value={ap} onChange={function(e){sAp(e.target.value);}}/><p className="hp">Parts & materials used</p></div>
      </div>
      {(actH||actP)>0&&<div className="tbox" style={{background:variance>500?"#5c1212":"var(--md)"}}><span>Actual total: {fmt$(actH*75+actP)}</span><span style={{fontSize:12}}>Variance vs est: {variance>=0?"+":""}{fmt$(variance)}</span></div>}
      ...
      <div className="fi"><label>Completion notes</label><textarea value={cn} onChange={function(e){sCn(e.target.value);}} rows={3} placeholder="Any notes on the job, delays, issues…"/></div>
    </div>
    {st==="e"&&<div className="err">Something went wrong.</div>}
...
export default function App(){
  ...
  var mm={
    ...
    review:{t:"Shop Review",d:"Enter estimate for "+(params.job||"…")},
    ...
    dashboard:{t:"Department Dashboard",d:"Live overview of all jobs, hours, and revenue"},
  };
